...
<div class="wrapper">
  ...
  require_once("../includes/initialize.php");
 $query ="SELECT g.`GUESTID`, `G_FNAME`, `G_LNAME`, `G_ADDRESS`,`CONFIRMATIONCODE`, `TRANSDATE`, `ARRIVAL`, `DEPARTURE`, `RPRICE` FROM `tblguest` g ,`tblreservation` r WHERE g.`GUESTID`=r.`GUESTID` and `CONFIRMATIONCODE` ='".$_GET['code']."'";
  $mydb->setQuery($query);
    ...
    <section class="invoice">
      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            <i class="fa fa-globe"></i> Tanchuling Hotel
      ...
      <div class="row invoice-info">
        <div class="col-sm-4 invoice-col">
          From
          <address>
            <strong>Tanchuling Hotel </strong><br>
            Email: info@tanchulinghotel.com
          </address>
        </div>
        <!-- /.col -->
        <div class="col-sm-4 invoice-col">
...
<br>
 ...
 $query ="SELECT * FROM `tblaccomodation` A,`tblroom`  RM, `tblreservation` RS  WHERE  A.`ACCOMID`=RM.`ACCOMID` AND RM.`ROOMID`=RS.`ROOMID`  and `CONFIRMATIONCODE` ='".$_GET['code']."'";
  $mydb->setQuery($query);
        ...
        <div class="col-xs-12 table-responsive">
          ...
          $days =  dateDiff(date($result->ARRIVAL),date($result->DEPARTURE));
            ...
            <tr> 
              ...
              <td> &#8369 <?php echo ($result->RPRICE*$days);?></td>
            </tr>
            
            
            <?php 
              @$tot += ($result->RPRICE*$days);
            } ?>
            </tbody>
          </table>
          <br><br>
           <b>Transaction Status: <?= $result->STATUS; ?></b>
        </div>
        <!-- /.col -->
      </div>
      <!-- /.row -->
